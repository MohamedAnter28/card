...
function detectCardType(number) {
  ...
  if (/^4/.test(first4)) return 'Visa';
  if (
    /^5[1-5]/.test(first4) ||
    /^2(2[2-9]|2[3-9]|[3-6]|7[01]|720)/.test(first4)
  )
    return 'MasterCard';
  if (/^3[47]/.test(first4)) return 'American Express';
  if (/^6(?:011|5[0-9]{2})/.test(first4)) return 'Discover';
  if (/^35(2[89]|[3-8][0-9])/.test(first4)) return 'JCB';
  if (/^(5018|5020|5038|5078|6304|6759|6761|6763)/.test(first4))
    return 'Maestro';
  return 'Unknown';
}
...
export default function Cards() {
  ...
  const handleAddOrEditCard = (e) => {
    e.preventDefault();
    // Validate card number for detected type (first 4 digits only)
    const number = form.number;
    const type = detectCardType(number);
    if (type === 'Unknown') {
      toast.showToast(
        'Please enter a valid Visa, MasterCard, or Maestro card (first 4 digits).',
        'error'
      );
      return;
    }
